...
class QLoRAEngine(TrainingEngine):
    def prepare(self) -> None:
        ...
        # Prepare for kbit training (layer norms in fp32, gradient checkpointing setup)
        # We must prevent running this twice on the same base model to avoid duplicate hooks causing SIGABRT
        if not getattr(self.model, "_is_kbit_prepared", False):
            self.model = prepare_model_for_kbit_training(
                self.model,
                use_gradient_checkpointing=self.cfg.gradient_checkpointing
            )
            self.model._is_kbit_prepared = True
    ...
    def train(self) -> None:
        ...
        sft_config = SFTConfig(
            ...
            logging_steps=10,
            report_to="none",  # Disable wandb for local MVP
        )

        # Handle train/val split if exists
        ...
        try:
            ...
            # Handle OOM gracefully for UI to catch
